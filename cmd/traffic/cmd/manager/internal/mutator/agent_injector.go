...
func addAgentContainer(
	...
	skips := cmpopts.IgnoreFields(core.Container{}, "ImagePullPolicy", "TerminationMessagePath", "TerminationMessagePolicy")

	refPodName := pod.Name + "." + pod.Namespace
	for i := range pod.Spec.Containers {
		pcn := &pod.Spec.Containers[i]
		if pcn.Name == agent.ContainerName {
			if cmp.Equal(pcn, acn, cmp.Comparer(compareProbes), cmp.Comparer(compareVolumeMounts), skips) {
				dlog.Infof(ctx, "Pod %s already has container %s and it isn't modified", refPodName, agent.ContainerName)
				return patches
			}
